#!/usr/bin/env bash
# Sync / verify the vendored frontend assets under web/, against the releases
# pinned in vendor.txt.
#
...
# check for "Leaflet cut a release" is a check you learn to ignore.
set -euo pipefail

cd "$(dirname "$0")/.."
# vendor.txt lives at the repo root rather than inside $dir: web/ is the
# published document root, and the pin list is not part of the game.
readonly dir=web
readonly list=vendor.txt
